...
{
	uint32_t key = 0;
	struct lab04_chainer_state state = {};
			   ...
			   const char *pin_dir)
{
	...
	if ((err = chainer_prog_path(prog_pin, sizeof(prog_pin), pin_dir)) != 0)
		return err;
	if ((err = state_map_path(map_pin, sizeof(map_pin), pin_dir)) != 0)
		return err;
	...
	prog = bpf_object__find_program_by_name(obj, "sockops_chainer");
	if (!prog) {
		err = -ENOENT;
		fprintf(stderr, "missing program sockops_chainer in %s\n", obj_path);
		goto out;
	}

	map = bpf_object__find_map_by_name(obj, "chainer_state");
	if (!map) {
		err = -ENOENT;
		fprintf(stderr, "missing map chainer_state in %s\n", obj_path);
		goto out;
	}
	...
	if ((err = pin_program(prog, prog_pin)) != 0)
		goto out;
	if ((err = pin_map(map, map_pin)) != 0)
		goto out;
	if ((err = reset_state_fd(map_fd)) != 0)
		goto out;

	printf("loaded chainer\n");
	printf("cgroup=%s\n", cgroup_path);
	printf("pin_dir=%s\n", pin_dir);
...
static int parse_slot(const char *raw, uint32_t *slot)
{
	...
	if (errno || !end || *end != '\0' || value > 3) {
		fprintf(stderr, "slot must be 0..3: %s\n", raw);
		return -EINVAL;
	}
			 ...
			 const char *prog_name, const char *slot_raw)
{
	...
	/*
	 * Reusing a slot is a replace operation in the tutorial. Removing the
	 * old link detaches the old freplace program before the new one is pinned.
	 */
	unlink(link_pin);
	unlink(prog_pin);
